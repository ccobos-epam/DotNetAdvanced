...
using BusinessLayer;
using CatalogService.Product.Get;
using BusinessLayer.Product.Service;
using BusinessLayer.Product.RR.Create;

namespace CatalogService.Product.Create;

public class CreateEndpoint_V01 : FE.Endpoint<ProductRequest_V01, ProductResponse_V01>
{
    ...
    public override void Configure()
    {
        ...
        {
            b.Accepts<ProductRequest_V01>("application/json+custom");
            b.Produces<ProductResponse_V01>(201, "application/json+custom");
        };
        ...
        Action<FE.EndpointSummary> es = s =>
        {
            ...
            s.ResponseExamples[201] = new ProductResponse_V01
            {
                Id = Guid.NewGuid(),
                Name = "New Product Name",
                ImageUrl = @"https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.istockphoto.com%2Fes%2Ffotos%2Famor&psig=AOvVaw0CUb7IMmNazRRlJNHUbZd2&ust=1763487208730000&source=images&cd=vfe&opi=89978449&ved=0CBUQjRxqFwoTCNifhPXb-ZADFQAAAAAdAAAAABAE",
    ...
    public override async Task HandleAsync(ProductRequest_V01 request,  CancellationToken cancellationToken)
    {
        var response = await ProductService.CreateProduct(request);
        await Send.CreatedAtAsync<GetEndpoint_V01>(response.Id,response, FE.Http.GET, null, false, default);
    }
}
